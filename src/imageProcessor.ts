...
export class ImageProcessor {
    ...
    private static loadImage(url: string): Promise<HTMLImageElement> {
        return new Promise((resolve, reject) => {
            ...
            img.onerror = () => {
                ...
                fallbackImg.onerror = (err) => reject(new Error(`Asset payload load error: ${err}`));
            };
        });
    }
    ...
    public static async analyze(
        ...
        // 1. Build Painting Texture Canvas Layer
        const bgCanvas = document.createElement("canvas");
        bgCanvas.width = screenW; bgCanvas.height = screenH;
        ...
        if (hasBgImage) {
            ...
            bgCtx.drawImage(img, 0, 0, screenW, screenH);
        } else {
            bgCtx.fillStyle = "#0b131a";
            bgCtx.fillRect(0, 0, screenW, screenH);
        }
        const highResBgData = bgCtx.getImageData(0, 0, screenW, screenH).data;

        // 2. Build Centered Typography Mask Layer
        const textCanvas = document.createElement("canvas");
        textCanvas.width = screenW; textCanvas.height = screenH;
        const textCtx = textCanvas.getContext("2d", { willReadFrequently: true })!;
        textCtx.fillStyle = "#000000";
        textCtx.fillRect(0, 0, screenW, screenH);
        ...
        if (hasFgText) {
            ...
            const optimalAutoFontSize = Math.min((screenW / maxLineLen) * 1.2, (screenH / (lines.length * 1.4)) * 0.75);
            ...
            const startY = (screenH / 2) - ((lines.length - 1) * lineHeight) / 2;
            lines.forEach((line, index) => {
                textCtx.fillText(line, screenW / 2, startY + index * lineHeight);
            });
        }
        const highResTextData = textCtx.getImageData(0, 0, screenW, screenH).data;

        // =================================================================
        // MATRIX COMPUTING FIELDS PARSING PASS ($128 \times 128$)
        // =================================================================
        ...
        lowResBgCtx.drawImage(bgCanvas, 0, 0, screenW, screenH, 0, 0, fCols, fRows);
        ...
        lowResTextCtx.drawImage(textCanvas, 0, 0, screenW, screenH, 0, 0, fCols, fRows);
        ...
        if (hasBgImage) {
            ...
            const tileW = screenW / columns; const tileH = screenH / rows;
            ...
            for (let r = 0; r < rows; r++) {
                for (let c = 0; c < columns; c++) {
                    ...
                    const hIdx = (Math.floor(cy) * screenW + Math.floor(cx)) * 4;
        ...
        if (hasFgText) {
            ...
            for (let y = 0; y < screenH; y += 4) {
                for (let x = 0; x < screenW; x += 4) {
                    if (highResTextData[(y * screenW + x) * 4] > 120) {
                        validTextCoords.push({ x, y });
                    }
                }
            }
            if (validTextCoords.length === 0) validTextCoords.push({ x: screenW / 2, y: screenH / 2 });
